...
Tokenizer::Tokenizer(std::string filename, std::string content)
    : mAccessMode(provideAccessMode()), mContent(std::move(content)),
      mFilename(std::move(filename)),
      mLanguageFeatureState(provideLanguageFeatures()),
      mKeyword(provideKeyWords()), mMemoryLayout(provideMemoryLayout()),
      mModifier(provideModifier()), mMutability(provideMutability()),
      mReservedWord(provideReservedWords()), mType(provideAtomicTypes()) {}

void Tokenizer::addToken(const std::string &con,
  ...
  case '.': {
    type = Token::Type::OPERATOR_SCOPE;
  } break;
  case ',': {
    type = Token::Type::COMMA;
  } break;
  case '{': {
    type = Token::Type::BRACKET_CURLY_OPEN;
  } break;
  case '}': {
    type = Token::Type::BRACKET_CURLY_CLOSE;
  } break;
  case '[': {
  ...
  case '=': {
    category = Token::Category::Assignment;
    type = Token::Type::ASSIGN;
  } break;
  case '<': {
    category = Token::Category::Operator;
    type = Token::Type::COMPARE_LESS;
  } break;
  case '>': {
    category = Token::Category::Operator;
    type = Token::Type::COMPARE_GREATER;
  } break;
  case '&': {
    category = Token::Category::Operator;
    type = Token::Type::BITAND;
  } break;
  case '|': {
    category = Token::Category::Operator;
    type = Token::Type::BITOR;
  } break;
  case '+': {
    category = Token::Category::Operator;
    type = Token::Type::MATH_ADDITION;
  } break;
  case '/': {
    category = Token::Category::Operator;
    type = Token::Type::MATH_DIVIDE;
  } break;
  case '%': {
    category = Token::Category::Operator;
    type = Token::Type::MATH_MODULO;
  } break;
  case '*': {
    category = Token::Category::Operator;
    type = Token::Type::MATH_MULTIPLY;
  } break;
  case '-': {
    category = Token::Category::Operator;
    type = Token::Type::MATH_SUBTRACT;
  } break;
  case '?': {
    category = Token::Category::Operator;
  ...
  default:
    if (content == OPERATOR_IS) {
      category = Token::Category::Operator;
      type = Token::Type::OPERATOR_IS;
    } else if (content == OPERATOR_RANGE) {
      category = Token::Category::Operator;
      type = Token::Type::OPERATOR_RANGE;
    } else if (isAccessMode(content)) {
      category = Token::Category::Modifier;
      type = Token::Type::ACCESS_MODE;
    } else if (isBoolean(content)) {
      category = Token::Category::Constant;
      type = Token::Type::CONST_BOOLEAN;
      ...
      category = Token::Category::Constant;
      type = Token::Type::CONST_FLOAT;
      // remove trailing 'f' character
      content = con.substr(0, con.length() - 1);
    } else if (isIntegerWithType(content)) {
      category = Token::Category::Constant;
      type = Token::Type::CONST_INTEGER;
    ...
    } else if (isLanguageFeature(content)) {
      category = Token::Category::Modifier;
      isOptional = true;
      type = Token::Type::LANGUAGE_FEATURE_STATE;
    } else if (isLiteral(content)) {
      category = Token::Category::Constant;
      type = Token::Type::CONST_LITERAL;
      // remove leading and trailing quotation marks (")
      content = con.substr(1, con.length() - 2);
    } else if (isMemoryLayout(content)) {
      category = Token::Category::Modifier;
      isOptional = true;
      type = Token::Type::MEMORY_LAYOUT;
    } else if (isModifier(content)) {
      category = Token::Category::Modifier;
      isOptional = true;
      type = Token::Type::MODIFIER;
    } else if (isMutability(content)) {
      category = Token::Category::Modifier;
      isOptional = true;
      type = Token::Type::MUTABILITY;
    } else if (isReservedWord(content)) {
      category = Token::Category::ReservedWord;
      type = Token::Type::RESERVED_WORD;
    } else if (isVisibility(content)) {
      category = Token::Category::Modifier;
      isOptional = true;
      type = Token::Type::VISIBILITY;
    }
    break;
  }
...
/*
 * mergeOperators: merges all pairs of & or | operators together (i.e. '&' '&'
 * become '&&') mergeOperators: merges all pairs of + or - operators together
 * (i.e. '+' '+' become '++') mergeOperators: merges all pairs of < or >
 * operators together (i.e. '<' '<' become '<<') mergeOperators: merges all
 * pairs of [ or ] operators together (i.e. '[' ']' become '[]')
 */
void Tokenizer::mergeOperators() {
  ...
  while (token != mTokens.end()) {
    ...
    if ((lastType == Token::Type::BITAND) &&
        (activeType == Token::Type::BITAND)) {
      // && and
      changed = true;
      // remove last added token ...
      tmp.pop_back();
      // ... and add AND instead
      tmp.emplace_back(Token::Category::Operator, Token::Type::AND, "&&",
                       token->position());
    } else if ((lastType == Token::Type::BITOR) &&
               (activeType == Token::Type::BITOR)) {
      // || or
      changed = true;
      // remove last added token ...
      tmp.pop_back();
      // ... and add OR instead
      tmp.emplace_back(Token::Category::Operator, Token::Type::OR, "||",
                       token->position());
    } else if ((lastType == Token::Type::OPERATOR_NOT) &&
               (activeType == Token::Type::BITAND)) {
      // !& negative and
      changed = true;
      // remove last added token ...
      tmp.pop_back();
      // ... and add OR instead
      tmp.emplace_back(Token::Category::Operator, Token::Type::NAND, "!&",
                       token->position());
    } else if ((lastType == Token::Type::OPERATOR_NOT) &&
               (activeType == Token::Type::BITOR)) {
      // !| negative or
      ...
      tmp.pop_back();
      // ... and add OR instead
      tmp.emplace_back(Token::Category::Operator, Token::Type::NOR, "!|",
                       token->position());
    } else if ((lastType == Token::Type::OPERATOR_SCOPE) &&
               (activeType == Token::Type::OPERATOR_SCOPE)) {
      // .. range operator
      changed = true;
      // remove last added token ...
      tmp.pop_back();
      // ... and add OR instead
      tmp.emplace_back(Token::Category::Operator, Token::Type::OPERATOR_RANGE,
                       OPERATOR_RANGE, token->position());
    } else if ((lastType == Token::Type::MATH_SUBTRACT) &&
               (activeType == Token::Type::MATH_SUBTRACT)) {
      // --
      changed = true;
      // remove last added token ...
      tmp.pop_back();
      // ... and add OR instead
      tmp.emplace_back(Token::Category::Operator,
                       Token::Type::OPERATOR_DECREMENT, "--",
                       token->position());
    } else if ((lastType == Token::Type::MATH_ADDITION) &&
               (activeType == Token::Type::MATH_ADDITION)) {
      // ++
      changed = true;
      // remove last added token ...
      tmp.pop_back();
      // ... and add AND instead
      tmp.emplace_back(Token::Category::Operator,
                       Token::Type::OPERATOR_INCREMENT, "++",
                       token->position());
    } else if ((lastType == Token::Type::COMPARE_LESS) &&
               (activeType == Token::Type::COMPARE_LESS)) {
      // <<
      changed = true;
      // remove last added token ...
      tmp.pop_back();
      // ... and add AND instead
      tmp.emplace_back(Token::Category::Operator,
                       Token::Type::OPERATOR_SHIFT_LEFT, "<<",
                       token->position());
    } else if ((lastType == Token::Type::COMPARE_GREATER) &&
               (activeType == Token::Type::COMPARE_GREATER)) {
      // >>
      changed = true;
      // remove last added token ...
      tmp.pop_back();
      // ... and add AND instead
      tmp.emplace_back(Token::Category::Operator,
                       Token::Type::OPERATOR_SHIFT_RIGHT, ">>",
                       token->position());
    } else if ((lastType == Token::Type::BRACKET_OPEN) &&
               (activeType == Token::Type::BRACKET_CLOSE)) {
      // && and
      changed = true;
      // remove last added token ...
      tmp.pop_back();
      // ... and add AND instead
      tmp.emplace_back(Token::Category::Operator, Token::Type::ARRAY_SUBSCRIPT,
                       "[]", token->position());
    }
...
void Tokenizer::process() {
  ...
  while (offset <= size) {
    ...
    else if (!isMultiLineComment && !isSingleLineComment && !isString &&
    ...
    if (!isMultiLineComment && !isSingleLineComment) {
      ...
      if (!isEscapeSequence && currentChar) {
        if (!isString &&
            DELIMITERS.find_first_of(currentChar) != std::string::npos) {
          if (!token.empty()) {
            if (currentChar == '"') {
              token += currentChar;
              currentChar = 0;
  ...
  mergeAssignments(); // replace assignment tokens with compare tokens (if
                      // present)
  mergeOperators();   // merge '+' '+' into '++'
  // replaceConstDataTypes();		// combines CONST_INTEGER '.'
  // CONST_INTEGER <data type> into a CONST_FLOAT or CONST_DOUBLE
  replaceConstDataTypesPatternMatching(); // combines CONST_INTEGER '.'
                                          // CONST_INTEGER <data type> into a
                                          // CONST_FLOAT or CONST_DOUBLE
  replaceOperators(); // combine 'operator' identifiers with the next following
                      // token i.e. 'operator' '+' => 'operator+'
}

/*
 * mergeAssignments: replaces/merges all assignment tokens with compare tokens
 * (if present), i.e. '+' & '=' => '+='
 */
void Tokenizer::mergeAssignments() {
  ...
  while (token != mTokens.end()) {
    ...
    if ((lastType == Token::Type::ASSIGN) &&
        (currentType == Token::Type::ASSIGN)) {
      // ==
      changed = true;
      // remove last added token ...
      tmp.pop_back();
      // ... and add COMPARE_EQUAL instead
      tmp.emplace_back(Token::Category::Comparator, Token::Type::COMPARE_EQUAL,
                       "==", token->position());
    } else if ((lastType == Token::Type::COMPARE_EQUAL) &&
               (currentType == Token::Type::ASSIGN)) {
      // ===
      changed = true;
      // remove last added token ...
      tmp.pop_back();
      // ... and add COMPARE_EQUAL instead
      tmp.emplace_back(Token::Category::Comparator,
                       Token::Type::COMPARE_EQUAL_CONTENT,
                       "===", token->position());
    } else if ((lastType == Token::Type::COMPARE_UNEQUAL) &&
               (currentType == Token::Type::ASSIGN)) {
      // !==
      changed = true;
      // remove last added token ...
      ...
      // ... and add COMPARE_EQUAL instead
      tmp.emplace_back(Token::Category::Comparator,
                       Token::Type::COMPARE_UNEQUAL_CONTENT,
                       "!==", token->position());
    } else if ((lastType == Token::Type::BITAND) &&
               (currentType == Token::Type::ASSIGN)) {
      // &=
      changed = true;
      // remove last added token ...
      ...
      tmp.emplace_back(Token::Category::Assignment, Token::Type::ASSIGN_BITAND,
                       "&=", token->position());
    } else if ((lastType == Token::Type::BITCOMPLEMENT) &&
               (currentType == Token::Type::ASSIGN)) {
      // ~=
      changed = true;
      // remove last added token ...
      ...
      tmp.emplace_back(Token::Category::Assignment,
                       Token::Type::ASSIGN_BITCOMPLEMENT,
                       "~=", token->position());
    } else if ((lastType == Token::Type::BITOR) &&
               (currentType == Token::Type::ASSIGN)) {
      // |=
      ...
      tmp.pop_back();
      // ... and add ASSIGN_ADD instead
      tmp.emplace_back(Token::Category::Assignment, Token::Type::ASSIGN_BITOR,
                       "|=", token->position());
    } else if ((lastType == Token::Type::OPERATOR_NOT) &&
               (currentType == Token::Type::ASSIGN)) {
      // !=
      ...
      // ... and add ASSIGN_ADD instead
      tmp.emplace_back(Token::Category::Comparator,
                       Token::Type::COMPARE_UNEQUAL, "!=", token->position());
    } else if ((lastType == Token::Type::GREATER ||
                lastType == Token::Type::COMPARE_GREATER) &&
               (currentType == Token::Type::ASSIGN)) {
      // >=
      changed = true;
      // remove last added token ...
      ...
      tmp.emplace_back(Token::Category::Comparator,
                       Token::Type::COMPARE_GREATER_EQUAL,
                       ">=", token->position());
    } else if ((lastType == Token::Type::LESS ||
                lastType == Token::Type::COMPARE_LESS) &&
               (currentType == Token::Type::ASSIGN)) {
      // <=
      changed = true;
      // remove last added token ...
      ...
      // ... and add ASSIGN_ADD instead
      tmp.emplace_back(Token::Category::Assignment,
                       Token::Type::ASSIGN_ADDITION, "+=", token->position());
    } else if ((lastType == Token::Type::MATH_DIVIDE) &&
               (currentType == Token::Type::ASSIGN)) {
      // /=
      changed = true;
      // remove last added token ...
      ...
      tmp.emplace_back(Token::Category::Assignment, Token::Type::ASSIGN_DIVIDE,
                       "/=", token->position());
    } else if ((lastType == Token::Type::MATH_MODULO) &&
               (currentType == Token::Type::ASSIGN)) {
      // %=
      changed = true;
      // remove last added token ...
      ...
      tmp.emplace_back(Token::Category::Assignment, Token::Type::ASSIGN_MODULO,
                       "*=", token->position());
    } else if ((lastType == Token::Type::MATH_MULTIPLY) &&
               (currentType == Token::Type::ASSIGN)) {
      // *=
      changed = true;
      // remove last added token ...
      ...
      // ... and add ASSIGN_MULTI instead
      tmp.emplace_back(Token::Category::Assignment,
                       Token::Type::ASSIGN_MULTIPLY, "*=", token->position());
    } else if ((lastType == Token::Type::MATH_SUBTRACT) &&
               (currentType == Token::Type::ASSIGN)) {
      // -=
      changed = true;
      ...
      tmp.emplace_back(Token::Category::Assignment,
                       Token::Type::ASSIGN_SUBTRACT, "-=", token->position());
    }
...
/*
 * This merges all integer consts with their following data type (also looks at
 * the scope token to produce doubles and floats i.e. 17.3f) CONST_INTEGER 'f':
 * 17 'f' CONST_INTEGER '.' 'f': 17 '.' 'f' CONST_INTEGER '.' CONST_INTEGER 'f':
 * 17 '.' 3 'f'
 */
    ...
    if (token->type() == Token::Type::CONST_INTEGER) {
      ...
      if (numCombines > 0) {
        ...
        while (numCombines > 0) {
          ...
          opToken->resetContentTo(opToken->content() +
                                  token->content()); // combine token contents
...
void Tokenizer::replaceConstDataTypesPatternMatching() {
  ...
  std::vector<TokenList> patterns{// 1 . 2 d
                                  TokenList{Token(Token::Type::CONST_INTEGER),
  ...
  while (token != mTokens.end()) {
    ...
    for (const TokenList &p : patterns) {
      patternIt = std::search(
          mTokens.begin(), mTokens.end(), p.begin(), p.end(),
      ...
      while (numCombines > 0) {
        ...
        if (patternIt->type() == Token::Type::IDENTIFIER) {
          if (patternIt->content() == "d") {
            ...
            throw Designtime::Exceptions::SyntaxError("invalid number format",
                                                      patternIt->position());
          }
        ...
        opToken->resetContentTo(opToken->content() +
                                patternIt->content()); // combine token contents
...
/*
 * This merges all 'operator' tokens with their according operator (+, -, *, /,
 * &, |, =, ==, <, <=, >, >=, etc.)
 */
void Tokenizer::replaceOperators() {
  ...
  while (token != mTokens.end()) {
    if (last != mTokens.end() &&
        (last->category() == Token::Category::Assignment ||
         last->category() == Token::Category::Operator) &&
        token->type() == Token::Type::RESERVED_WORD &&
        token->content() == RESERVED_WORD_OPERATOR) {
      // we found an operator
      ...
      mTokens.erase(last);
    } else if (last != mTokens.end() &&
               last->type() == Token::Type::RESERVED_WORD &&
               last->content() == RESERVED_WORD_OPERATOR) {
...
const TokenList &Tokenizer::tokens() const { return mTokens; }
