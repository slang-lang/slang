...
SemanticVersionNumber::SemanticVersionNumber(int major, int minor, int bugfix,
                                             const std::string &label,
                                             const std::string &buildMetadata)
    : mBugfix(bugfix), mBuildMetadata(buildMetadata), mLabel(label),
...
bool SemanticVersionNumber::operator==(
    const SemanticVersionNumber &other) const {
  return mMajor == other.mMajor && mMinor == other.mMinor &&
         mBugfix == other.mBugfix && mLabel == other.mLabel &&
...
bool SemanticVersionNumber::isValid() const { return mIsValid; }
...
void SemanticVersionNumber::parse(std::string version) {
  ...
  mIsValid = mMajor != 0 || mMinor != 0 || mBugfix != 0 || !mLabel.empty() ||
             !mBuildMetadata.empty();
}

std::string SemanticVersionNumber::toString() const {
  return Utils::Tools::toString(mMajor) + "." + Utils::Tools::toString(mMinor) +
