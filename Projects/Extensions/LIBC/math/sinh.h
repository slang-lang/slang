...
#ifndef Slang_Extensions_LIBC_math_sinh_h
#define Slang_Extensions_LIBC_math_sinh_h
...
class SINH: public ExtensionMethod
{
public:
	SINH()
...
class SINHF: public ExtensionMethod
{
public:
	SINHF()
	: ExtensionMethod(0, "sinhf", Designtime::FloatObject::TYPENAME)
	{
		ParameterList params;
		params.push_back(Parameter::CreateDesigntime("arg", Common::TypeDeclaration(Designtime::StringObject::TYPENAME)));
...
public:
	Runtime::ControlFlow::E execute(Common::ThreadId threadId, const ParameterList& params, Runtime::Object* result, const Token& token)
	{
		...
		try {
			...
			*result = Runtime::FloatObject(sinhf(param_arg));
		}
		catch ( std::exception& e ) {
			Runtime::Object *data = Controller::Instance().repository()->createInstance(Runtime::StringObject::TYPENAME);
			*data = Runtime::StringObject(std::string(e.what()));
