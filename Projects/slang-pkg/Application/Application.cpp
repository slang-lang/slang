...
void prepareModuleInstallation(const std::string &repo,
...
void update();
size_t upload(const std::string &filename, const std::string &url,
...
int Application::exec() {
  ...
  case Create:
    create(mParameters);
    break;
  case CreateLocalLibrary:
    createLocalLibrary();
    break;
  case Help:
    printUsage(mParameters);
    break;
  case Info:
    info(mParameters);
    break;
  ...
  case Remove:
    ...
    break;
  case Restrict:
    processRestrictions(mParameters);
    break;
  case Search:
    search(mParameters);
    break;
...
void Application::printUsage(const StringList &params) {
  if (!params.empty()) {
    ...
    if (param == "create") {
      ...
      std::cout << std::endl;
      std::cout
          << "[Deprecated] Provide a single directory name to create a module "
             "from it. A module.json is required in this directory."
          << std::endl;
    } else if (param == "help") {
      std::cout << "Usage: slang-pkg help [<command>]" << std::endl;
      std::cout << std::endl;
      std::cout << "Prints out the usage/info for a given command."
                << std::endl;
    } else if (param == "info") {
      ...
      std::cout << "Usage: slang-pkg install <module> [<module> ...]"
                << std::endl;
      std::cout << std::endl;
      std::cout << "Downloads and installs given module[s] from remote "
                   "repository into library folder."
                << std::endl;
    } else if (param == "list") {
      std::cout << "Usage: slang-pkg list" << std::endl;
      std::cout << std::endl;
      std::cout << "Lists installed modules (including version number)."
                << std::endl;
    } else if (param == "purge") {
      std::cout << "Usage: slang-pkg purge <module> [<module> ...]"
                << std::endl;
      std::cout << std::endl;
      std::cout << "Deletes installed module[s] and removes restrictions."
                << std::endl;
    } else if (param == "push") {
      std::cout << "Usage: slang-pkg push <module> [<module> ...]" << std::endl;
      std::cout << std::endl;
      std::cout << "Creates a module and pushes it to the configured "
                   "repository server."
                << std::endl;
    } else if (param == "remove") {
      std::cout << "Usage: slang-pkg remove <module> [<module> ...]"
                << std::endl;
      std::cout << std::endl;
      std::cout << "Deletes installed module[s] without removing restrictions."
                << std::endl;
    } else if (param == "restrict") {
      std::cout << "Usage: slang-pkg restrict <module> [ "
                   "[<version_min>]:[<version_max>] ]"
                << std::endl;
      std::cout << std::endl;
      std::cout << "Adds or removes a restriction for a given module."
                << std::endl;
      std::cout << std::endl;
      std::cout << "Example to add a minimum version restriction without "
                   "setting a maximum version:"
                << std::endl;
      std::cout << "    slang-pkg restrict System 0.1.0:" << std::endl;
    } else if (param == "search") {
      std::cout << "Usage: slang-pkg search <text>" << std::endl;
      std::cout << std::endl;
      std::cout << "Searches the module cache for the given parameter."
                << std::endl;
    } else if (param == "update") {
      std::cout << "Usage: slang-pkg update" << std::endl;
      std::cout << std::endl;
      std::cout
          << "Updates the module cache from the configured remote repository."
          << std::endl;
    } else if (param == "upgrade") {
      std::cout << "Usage: slang-pkg upgrade <module> [<module> ...]"
                << std::endl;
      std::cout << std::endl;
      std::cout << "Upgrades outdated modules according to the currently set "
                   "restrictions. Upgrades only the provided modules or, if no "
                   "parameter is provided, upgrades all outdated modules."
                << std::endl;
    } else if (param == "--locallibrary") {
      std::cout << "Usage: slang-pkg --locallibrary" << std::endl;
      std::cout << std::endl;
      std::cout
          << "Creates a library folder in the current folder which is used to "
             "install and update modules from a remote repository."
          << std::endl;
    } else if (param == "--skip-dependencies") {
      std::cout << "Usage: slang-pkg --skip-dependencies <command>"
                << std::endl;
      std::cout << std::endl;
      std::cout << "Flag to indicate if dependencies should be ignored while "
                   "installing or removing/purging modules."
                << std::endl;
    } else if (param == "--verbose") {
      std::cout << "Usage: slang-pkg <command> --verbose" << std::endl;
      std::cout << std::endl;
      std::cout << "Activates verbose output of given command, if supported."
                << std::endl;
      ...
      std::cerr << "!!! Unknown command '" << param << "' provided"
                << std::endl;
    }
  ...
  std::cout << std::endl;
  std::cout
      << "create                     Create a new module from a given directory"
      << std::endl;
  std::cout << "help                       Print command usage info"
            << std::endl;
  std::cout
      << "info                       Print information about requested module"
      << std::endl;
  std::cout << "install                    Install new module" << std::endl;
  std::cout << "list                       List all installed modules"
            << std::endl;
  std::cout << "purge                      Remove an installed module and all "
               "of its configuration"
            << std::endl;
  std::cout << "push                       Push module(s) to repository server"
            << std::endl;
  std::cout << "remove                     Remove an installed module"
            << std::endl;
  std::cout << "restrict                   Add version restriction for module"
            << std::endl;
  std::cout << "search                     Search for a module" << std::endl;
  std::cout << "update                     Update repository indices"
            << std::endl;
  std::cout << "upgrade                    Upgrade outdated modules"
            << std::endl;
  std::cout << "--locallibrary             Use current directory as library"
            << std::endl;
  std::cout << "--skip-dependencies        Skip dependencies while installing "
               "or removing modules"
            << std::endl;
...
void Application::printVersion() {
  std::cout << PRODUCT_NAME << " Dependency Manager " << PRODUCT_VERSION
            << " (cli) " << mSettings.Architecture << std::endl;
...
void checkOutdatedModules(Modules &outdatedModules) {
  ...
  // find all folders in local <repo> folder and compare their corresponding
  // <repo>/<module>/module.json [ version ] field with the version in the
  // index.json file
  ...
  for (const auto &localModule : local) {
    for (const auto &remoteModule : remote) {
      if (localModule.mShortName == remoteModule.mShortName &&
          localModule.mVersion < remoteModule.mVersion) {
...
bool checkRestrictions(const Module &module) {
  for (const auto &mLocalRestriction : mSettings.LocalRestrictions) {
    if (mLocalRestriction.mModule == module.mShortName) {
      ...
      if (mLocalRestriction.mMinVersion.isValid() &&
          module.mVersion < mLocalRestriction.mMinVersion) {
        return false;
      }
      // check maximum version
      if (mLocalRestriction.mMaxVersion.isValid() &&
          mLocalRestriction.mMaxVersion < module.mVersion) {
        return false;
      }
    }
  }
...
void cleanCache() {
  ...
  for (StringList::const_iterator it = mDownloadedFiles.begin();
       it != mDownloadedFiles.end(); ++it) {
    execute("rm " + (*it));
  }
}

void collectLocalModuleData() {
  // iterate over all directories in the modules directory and collect all
  // "module.json" files

  auto *dir = opendir(mSettings.LibraryFolder.c_str());
  if (!dir) {
    std::cerr << "!!! Error while accessing modules directory '"
              << mSettings.LibraryFolder << "'" << std::endl;
    exit(ERROR_MODULE_DIRECTORY);
  }

  struct dirent *dirEntry = readdir(dir);

  // add Slang version as virtually installed module to force correct
  // dependencies
  // {
  Module slang;
  slang.mArchitecture = mSettings.Architecture;
  slang.mDescription =
      "Virtual module to prevent installation of modules that are not "
      "compatible with your version of the Slang interpreter";
    ...
    if (dirEntry->d_type == DT_DIR) {
      ...
      if (entry != "." && entry != "..") {
        ...
        if (Utils::Tools::Files::exists(path + filename)) {
          mLocalRepository.addModule(collectModuleData(path, filename));
        }
      }
    }
...
Module collectModuleData(const std::string &path, const std::string &filename) {
  ...
  if (!module.loadFromJson(config)) {
    std::cerr << "!!! Invalid module data in file '" << filename << "'"
              << std::endl;
    return module;
  }
...
void create(const StringList &params) {
  ...
  { // create version specific module information ( "<module>_<version>.json" )
    std::cout << "Creating module information \""
              << module.mShortName + FILE_VERSION_SEPARATOR +
                     module.mVersion.toString() + ".json\""
              << std::endl;

    execute("cp " + path + "/" + filename + " " + module.mShortName +
            FILE_VERSION_SEPARATOR + module.mVersion.toString() + ".json");
  }
  { // create package ( "<module>_<version>.tar.gz" )
    std::cout << "Creating module package \""
              << module.mShortName + FILE_VERSION_SEPARATOR +
                     module.mVersion.toString() + ".tar.gz\""
              << std::endl;

    execute("tar --exclude-vcs -cjf " + module.mShortName +
            FILE_VERSION_SEPARATOR + module.mVersion.toString() + ".tar.gz " +
            path);
  }
}
...
void createLocalLibrary() {
  ...
  if (!Utils::Tools::Files::exists(mSettings.CurrentFolder +
  ...
  if (!Utils::Tools::Files::exists(mSettings.CurrentFolder + CONFIG_FILE)) {
    ...
    repository["authentication"] =
        "Bearer"; // user needs to fill in his preferred method of
                  // authentication (Basic|Bearer)
    repository["authorization"] =
        "<get-your-token-from-https://modules.slang-lang.org>"; // user needs to
                                                                // fill in his
                                                                // authorization
                                                                // token here
...
bool download(const std::string &url, const std::string &target,
              bool allowCleanup) {
#ifdef SLANG_DEBUG
  std::cout << "Downloading \"" << url << "\" => \"" << target << "\""
            << std::endl;
#endif
 ...
 */
size_t findCaseInsensitive(const std::string &data, const std::string &toSearch,
                           size_t pos) {
  ...
  std::transform(toSearchLower.begin(), toSearchLower.end(),
                 toSearchLower.begin(), ::tolower);
...
void info(const StringList &params) {
  ...
  for (auto paramIt = params.cbegin(); paramIt != params.cend(); ++paramIt) {
    ...
    Utils::Tools::splitBy((*paramIt), VERSION_SEPARATOR, moduleName,
                          moduleVersion);
    ...
    { // look up module in local repository
      ...
      for (const auto &localIt : localModules) {
        if (localIt.mShortName == moduleName &&
            localIt.mVersion == moduleVersion) {
          found = true;
    ...
    if (!found) { // lookup module in remote repository
      ...
      auto url = mRemoteRepository.getURL() + MODULES + moduleName +
                 FILE_VERSION_SEPARATOR + moduleVersion + ".json";
    ...
    if (found) { // print out module information if found
      std::cout << infoModule.mShortName << VERSION_SEPARATOR
                << infoModule.mVersion.toString() << " - "
                << infoModule.mLongName << std::endl;
      ...
      if (infoModule.mDependencies.empty()) {
        ...
        for (const auto &dependency : infoModule.mDependencies) {
          ...
          std::cout << "min: "
                    << (dependency.MinVersion.empty() ? "<not set>"
                                                      : dependency.MinVersion);
          std::cout << "\t\tmax: "
                    << (dependency.MaxVersion.empty() ? "<not set>"
                                                      : dependency.MaxVersion);
          std::cout << "\t\tsource: "
                    << (dependency.Source.empty() ? "<not set>"
                                                  : dependency.Source);
          std::cout << std::endl;
        }
      }
    } else {
      std::cerr << "!!! Requested module \"" << moduleName << "\" not found"
                << std::endl;
    }
  }
}
...
void install(const StringList &params) {
  ...
  for (const auto &param : params) {
    ...
    prepareModuleInstallation(mRemoteRepository.getURL(),
                              Module(moduleName, version, source));
  }

  bool allowInstall = true;

  // add all other requested modules to missing modules to prevent multiple
  ...
  for (const auto &param : params) {
    ...
    if (!checkRestrictions(missingModule)) {
      allowInstall = false;
      std::cout << "!!! Module " << missingModule.mShortName << "( "
                << missingModule.mVersion.toString()
                << " ) cannot be installed due to current module restrictions"
                << std::endl;
    }
    ...
    for (const auto &moduleIt : missing) {
      ...
      if (mLocalRepository.getModule(moduleIt.mShortName, tmp)) {
        if (!forceVersion && !(tmp.mVersion < moduleIt.mVersion)) {
          std::cout << "Same or newer version ( " << tmp.mVersion.toString()
                    << " vs " << moduleIt.mVersion.toString()
                    << " ) of module \"" << moduleIt.mShortName
...
void installModule(const std::string &repo, const std::string &module) {
  std::cout << "Installing module \"" << module << "\" from \"" << repo
            << "\"..." << std::endl;

  auto moduleConfig =
      mSettings.LibraryFolder + CONFIG_CACHE_MODULES + module + ".json";

  if (!Utils::Tools::Files::exists(moduleConfig)) {
    std::cerr << "!!! Module information missing for module \"" << module
              << "\"" << std::endl;
    return;
  }

  auto tmpModule = collectModuleData(
      mSettings.LibraryFolder + CONFIG_CACHE_MODULES, module + ".json");
  ...
  if (!config["target"].isMember("type")) {
    std::cerr << "!!! No type entry found in target module information"
              << std::endl;
    return;
  }
  ...
  if (type == "external") {
    ...
    url = repo + MODULES + config["name_short"].asString() + "/" +
          config["version"].asString() + "/module.tar.gz";
  } else {
    std::cerr << "!!! invalid target type specified" << std::endl;
    return;
  }

  auto module_archive = mSettings.LibraryFolder + CONFIG_CACHE_MODULES +
                        module + FILE_VERSION_SEPARATOR +
                        config["version"].asString() + ".tar.gz";
  ...
  for (const std::string &file : tmpModule.mTarget.InstalledFiles) {
    execute(CONFIG_SCRIPTS + "addLink.sh \"" + mSettings.LibraryFolder + "/" +
            module + "/" + file + "\" \"" + mSettings.LibraryFolder + "/" +
            CONFIG_APPS + "\"");
  }
}
...
void list() {
  ...
  for (const auto &localIt : local) {
    std::cout << localIt.mShortName << VERSION_SEPARATOR
              << localIt.mVersion.toString() << " - " << localIt.mLongName
              << std::endl;
  }
...
void loadConfig() {
  ...
  if (mSettings.Config.isMember("repository")) {
    ...
    if (entry["authentication"].asString() == "Basic") {
      authentication =
          Repository::Authentication::Basic; // not yet implemented on server,
                                             // so don't use it yet
    } else if (entry["authentication"].asString() == "Bearer") {
      authentication =
          Repository::Authentication::Bearer; // preferred way of authentication
    }
    // }
...
void loadRestrictions() {
  ...
  // add hardcoded max restriction for Slang version ( this can be overwritten
  // by the real restriction config )
...
void prepareModuleInstallation(const std::string &repo,
                               const Module &installModule) {
#ifdef SLANG_DEBUG
  std::cout << "Preparing module \"" << installModule.mShortName << " ("
            << installModule.mVersion.toString() << ")\" from \"" << repo
            << "\"..." << std::endl;
#endif

  // ( 1 ) download module information from repository
  // ( 2 ) collect dependencies from module information
  // ( 3 ) check dependencies against local repository and download module
  // information for missing modules

  auto path = mSettings.LibraryFolder + CONFIG_CACHE_MODULES;
  auto filename = installModule.mShortName + ".json";
  auto moduleConfig = path + filename;
  auto url = repo + MODULES + installModule.mShortName + "/" +
             installModule.mVersion.toString() + "/module.json";

  bool result = download(url, moduleConfig);
  if (!result) {
    std::cerr << "!!! Download of module information for \""
              << installModule.mShortName << "\" failed" << std::endl;
    return;
  }
  ...
  if (!module.mArchitecture.empty()) {
    if (module.mArchitecture != mSettings.Architecture) {
      std::cerr << "!!! module architecture " << module.mArchitecture
                << " does not match system architecture" << std::endl;
  ...
  for (const auto &dependency : module.mDependencies) {
    ...
    if (!found) {
      ...
      std::cout << "Need to install dependent module \"" << dependency.Module
                << "\"" << std::endl;

      Module dependent(dependency.Module, dependency.MinVersion,
                       dependency.Source);
...
bool prepareRemoteRepository() {
  auto filename = mSettings.LibraryFolder + CONFIG_CACHE_REPOSITORIES +
                  mRemoteRepository.getName() + ".json";
...
void processRestrictions(const StringList &params) {
  ...
  if (params.empty()) {
    std::cerr << "!!! Invalid number of parameters: " << params.size()
              << std::endl;
    return;
  }
  ...
  if (params.size() == 2) {
    ...
    if (!minVersion.empty())
      value["version_min"] = SemanticVersionNumber(minVersion).toString();
    if (!maxVersion.empty())
      value["version_max"] = SemanticVersionNumber(maxVersion).toString();
...
size_t push(const StringList &params) {
  ...
  // ( 1 ) check if files "<module>_<version>.json", "<module>_<version>.tar.gz"
  // are present ( 2 ) upload "<module>_<version>.json",
  // "<module>_<version>.tar.gz" into "upload" folder of the repository server
  ...
  for (const auto &param : params) {
    ...
    auto result =
        upload(module.mShortName + FILE_VERSION_SEPARATOR +
                   module.mVersion.toString() + ".json",
               mRemoteRepository.getURL() + UPLOAD_PATH, "application/json");

    if (result) {
      std::cerr << "!!! Error " << result
                << " while uploading module to repository '"
                << mRemoteRepository.getURL() << "'" << std::endl;
    } else {
      result = upload(module.mShortName + FILE_VERSION_SEPARATOR +
                          module.mVersion.toString() + ".tar.gz",
    ...
    if (!result) {
      std::cout << "Successfully uploaded module '" << param
                << "' to repository '" << mRemoteRepository.getURL() << "'"
                << std::endl;
    }

    // cleanup artifacts
    execute("rm " + module.mShortName + FILE_VERSION_SEPARATOR +
            module.mVersion.toString() + ".json");
    execute("rm " + module.mShortName + FILE_VERSION_SEPARATOR +
            module.mVersion.toString() + ".tar.gz");
  }
...
void readJsonFile(const std::string &filename, Json::Value &result) {
  ...
  std::string data((std::istreambuf_iterator<char>(stream)),
                   std::istreambuf_iterator<char>()); // read stream
  stream.close();
  ...
  for (const auto &param : params) {
    ...
    for (const auto &localIt : local) {
      if (localIt.mShortName == param) {
        ...
        std::cout << "Removing module \"" << localIt.mShortName << "\" from \""
                  << localIt.mTarget.Directory << "\"..." << std::endl;

        execute(CONFIG_SCRIPTS + "removeLinks.sh \"" + mSettings.LibraryFolder +
                "/" + CONFIG_APPS + "\" \"" + mSettings.LibraryFolder + "/" +
                localIt.mShortName + "\"");
    ...
    if (!found) {
      std::cerr << "!!! Module \"" << param
                << "\" cannot be removed because it is not installed"
                << std::endl;
    }
  }
}
...
void search(const StringList &params) {
  ...
  if (!config.isMember("modules")) {
    std::cerr << "!!! Invalid cache file structure: \"modules\" missing"
              << std::endl;
    return;
  }
  ...
  for (const auto &member : config["modules"]) {
    if (!member.isMember("name")) {
      std::cerr << "!!! Invalid cache file structure: \"name\" missing"
                << std::endl;
      return;
    }

    auto description =
        member.isMember("description") ? member["description"].asString() : "";
    ...
    if (member.isMember("keywords")) {
      ...
      for (const auto &keyword : member["keywords"]) {
        if (findCaseInsensitive(keyword.asString(), lookup) !=
            std::string::npos) {
          result.push_back(shortName + VERSION_SEPARATOR + version + " - " +
                           longName);
...
void update() {
  ...
  std::cout << "Updating repository \"" << mRemoteRepository.getName()
            << "\"..." << std::endl;
  ...
  if (result) {
    std::cout << "Updated index for \"" << mRemoteRepository.getURL() << "\""
              << std::endl;
  } else {
    std::cerr << "!!! Error while updating index for "
              << mRemoteRepository.getURL() << std::endl;
...
void upgrade(const StringList &params) {
  ...
  if (outdatedModules.empty()) {
    ...
    for (const auto &outdatedModule : outdatedModules) {
      // if params contains values upgrade only the modules that are set in
      // mParameters
    ...
    std::cout << "Need to upgrade " << upgradeModules.size() << " module(s)..."
              << std::endl;

    for (const auto &outdatedModule : upgradeModules) {
      std::cout << outdatedModule.mShortName << "( "
                << outdatedModule.mVersion.toString() << " )" << std::endl;
  ...
  if (curl) {
    ...
    { // set curl options
      ...
      curl_easy_setopt(curl, CURLOPT_NOPROGRESS,
                       1L); // disable progress meter, set to 0L to enable and
      ...
      curl_easy_setopt(curl, CURLOPT_VERBOSE,
                       static_cast<long>(
                           mSettings.Verbose)); // Switch on full protocol/debug
